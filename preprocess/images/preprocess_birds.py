...
import os
import pickle
from preprocess_utils import get_image
...
def save_data_list(inpath, outpath, filenames, filename_bbox):
    ...
    for key in filenames:
        ...
        f_name = '%s/segmentations/%s.png' % (inpath, key)
        img = get_image(f_name, LOAD_SIZE, is_crop=True, bbox=bbox)
        img = img.astype('uint8')
        hr_images.append(np.expand_dims(img, axis=-1))
        img = Image.fromarray(img)
        lr_img = img.resize(size=(lr_size, lr_size), resample=Image.BICUBIC)
        lr_img = np.array(lr_img)
        lr_images.append(np.expand_dims(lr_img, axis=-1))
        cnt += 1
        if cnt % 100 == 0:
            print('Load %d......' % cnt)
    #
    print('segmentations', len(hr_images), hr_images[0].shape, lr_images[0].shape)
    #
    outfile = outpath + str(LOAD_SIZE) + 'segmentations.pickle'
    with open(outfile, 'wb') as f_out:
        pickle.dump(hr_images, f_out)
        print('save to: ', outfile)
    #
    outfile = outpath + str(lr_size) + 'segmentations.pickle'
    with open(outfile, 'wb') as f_out:
