...
if __name__ == '__main__':
    ...
    for i in range(batch_size):
        sample = []
        np_image  = sample.append(np.expand_dims(np_images[i], 0))
        np_bimage = sample.append(np.expand_dims(np_bimages[i], 0))
        for j in range(2,6):
            sample.append(np.expand_dims(vis_samples[j][i].transpose(2,0,1), 0))
        # np_fake   = np.expand_dims(np_fakes[i], 0)
        # sample = [np_image, np_bimage, np_fake]
        # for k in sample:
        #     print(k.shape)
        # exit()
        save_images(sample, save=not sample_folder == '', save_path=os.path.join(sample_folder, '%d.png' % i), dim_ordering='th')
